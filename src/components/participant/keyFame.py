...
def start():
    ...
    while cap.isOpened():
        ...
        if ret == True:
            frame_rgb = cv2.cvtColor(frame,
                                     cv2.COLOR_BGR2RGB)  # since cv reads frame in bgr order so rearraning to get frames in rgb order
            ...
            arr = np.vstack((arr,
                             feature_vector))  # appending each one-dimensinal vector to generate N*M matrix (where N is number of frames
            # and M is 1944)
            count += 1
        else:
            break
    ...
    df['v7']= df['v7'].astype(int)  #converting the cluster level from float type to integer type
    df1 =  df[df.v7.isin(res)]   #filter only those frames which are eligible to be a part of shot or filter those frames who are
    #part of required clusters that have more than 25 frames in it
    new = df1.groupby('v7').tail(1)['v7'] #For each cluster /group take its last element which summarize the shot i.e key-frame
